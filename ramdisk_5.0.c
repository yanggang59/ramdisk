...
static blk_status_t read_ramdisk(struct page *page, unsigned int off, void *addr, unsigned int len)
{
	...
	while (len) {
		mem = kmap_atomic(page);
		chunk = min_t(unsigned int, len, PAGE_SIZE - off);
		rem = memcpy_mcsafe(mem + off, addr, chunk);
		kunmap_atomic(mem);
		if (rem)
...
static int ramdisk_rw_page(struct block_device *bdev, sector_t sector, struct page *page, unsigned int op)
{
	...
	blk_status_t rc;
	if (op_is_write(op))
		rc = ramdisk_do_write(r_dev, page, 0, sector, hpage_nr_pages(page) * PAGE_SIZE);
	else
		rc = ramdisk_do_read(r_dev, page, 0, sector, hpage_nr_pages(page) * PAGE_SIZE);
	if (rc == 0)
...
static blk_qc_t ramdisk_make_request(struct request_queue *q, struct bio *bio)
{
	...
	struct ramdisk_dev* r_dev = q->queuedata;
...
{
	struct gendisk *disk = g_ramdisk_dev.disk;
	del_gendisk(disk);
